...
public class FakeMoodLogRepository extends CrudRepositoryFake<MoodLog, Long>
    ...
    public Stream<MoodLog> findByUserIdOrderByCreatedAtDesc(Long userId) {
        return memory.values().stream()
                .filter(moodLog -> moodLog.getUser().getId().equals(userId))
                .sorted(Comparator.comparing(MoodLog::getCreationDate).reversed());
    }

    @Override
    public List<User> findUsersWhoDidNotVoteToday(long startOfDay, long endOfDay) {
        return memory.values().stream()
                .filter(moodLog -> moodLog.getCreationDate() >= startOfDay)
                .filter(moodLog -> moodLog.getCreationDate() <= endOfDay)
                .map(MoodLog::getUser)
                .distinct()
                .collect(Collectors.toList());
    }
    ...
    public List<MoodLog> findMoodLogsForWeek(Long userId, long weekStart) {
        return memory.values().stream()
                .filter(moodLog -> moodLog.getUser().getId().equals(userId))
                .filter(moodLog -> moodLog.getCreationDate() >= weekStart)
                .collect(Collectors.toList());
    }
    ...
    public List<MoodLog> findMoodLogsForMonth(Long userId, long monthStart) {
        return memory.values().stream()
                .filter(moodLog -> moodLog.getUser().getId().equals(userId))
                .filter(moodLog -> moodLog.getCreationDate() >= monthStart)
                .collect(Collectors.toList());
    }
} */
